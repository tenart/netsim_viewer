...
for(let i = 0; i < 10; i++) {
    ...
    let node = new Node();
    node.type = colors[i % colors.length];
    node.position = utils.pointAngleDistance(utils.screenCenter(), 36*i, 150);
...
simulator.onStart(() => {
    // clientA.send(clientB, 40000, simulator.time);
    // Always need to initialize view
    clients[0].send(clients[1], 40000, simulator.time);
    view.init();
})
...
clients.forEach((node, index) => {
    if(index !== clients.length-1) {
        node.onReceive(message => {
            node.send(clients[index+1], message.size, message.time.arrive);
        })
    } else {
        node.onReceive(message => {
            node.send(clients[0], message.size, message.time.arrive);
        })
    }
})
